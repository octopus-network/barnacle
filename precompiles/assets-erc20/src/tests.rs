...
fn selector_less_than_four_bytes() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_matches!(
			precompiles().execute(
				Account::ForeignAssetId(0u32).into(),
				&bogus_selector,
				None,
				&Context {
					address: Account::ForeignAssetId(1u32).into(),
		...
		.execute_with(|| {
			...
			assert_eq!(
				precompiles().execute(
					...
					None,
					&Context {
						address: Account::ForeignAssetId(0u32).into(),
						caller: Account::Alice.into(),
		...
		.execute_with(|| {
			...
			assert_eq!(
				...
				Some(Ok(PrecompileOutput {
					...
					cost: 30832756u64,
					logs: LogsBuilder::new(Account::ForeignAssetId(0u32).into())
						.log3(
		...
		.execute_with(|| {
			...
			));
			{
				assert_eq!(
					...
					Some(Ok(PrecompileOutput {
						exit_status: ExitSucceed::Returned,
						output: EvmDataWriter::new().write::<Bytes>("TestToken".into()).build(),
						cost: Default::default(),
						logs: Default::default(),
					}))
				);
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			));
			{
				assert_eq!(
					precompiles().execute(
						Account::LocalAssetId(0u32).into(),
						&EvmDataWriter::new_with_selector(Action::Mint)
							.write(Address(Account::Bob.into()))
							.write(U256::from(400))
							.build(),
						None,
						&Context {
							address: Account::LocalAssetId(0u32).into(),
							caller: Account::Alice.into(),
							apparent_value: From::from(0),
						},
						false,
					...
					Some(Ok(PrecompileOutput {
						...
						logs: LogsBuilder::new(Account::LocalAssetId(0u32).into())
							.log3(
								SELECTOR_LOG_TRANSFER,
								Account::Zero,
								Account::Bob,
