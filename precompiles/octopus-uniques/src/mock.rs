...
impl pallet_uniques::Config<pallet_uniques::Instance1> for Test {
	type RuntimeEvent = RuntimeEvent;
...
where
	OctopusUniquesPrecompile<R>: Precompile,
{
	fn execute(&self, handle: &mut impl PrecompileHandle) -> Option<EvmResult<PrecompileOutput>> {
		match handle.code_address() {
			a if a == hash(PRECOMPILE_ADDRESS) =>
				Some(OctopusUniquesPrecompile::<R>::execute(handle)),
			_ => None,
		}
	}
...
pub type PCall = OctopusUniquesPrecompileCall<Test>;
