...
// use pallet_octopus_support::traits::LposInterface;
...
where
	OctopusBridgePrecompile<R>: Precompile,
{
	fn execute(&self, handle: &mut impl PrecompileHandle) -> Option<EvmResult<PrecompileOutput>> {
		match handle.code_address() {
			a if a == hash(PRECOMPILE_ADDRESS) =>
				Some(OctopusBridgePrecompile::<R>::execute(handle)),
			_ => None,
		}
	}
...
pub type PCall = OctopusBridgePrecompileCall<Test>;
