		...
		.execute_with(|| {
			precompiles()
				.prepare_test(
					Account::Alice,
					Account::Precompile,
					PCall::approve { spender: Address(Account::Bob.into()), value: 500.into() },
				)
				.expect_cost(1756)
				.expect_log(log3(
					Precompile,
		...
		.execute_with(|| {
			precompiles()
				.prepare_test(
					Account::Alice,
					Account::Precompile,
					PCall::approve { spender: Address(Account::Bob.into()), value: U256::MAX },
				)
				.expect_cost(1756u64)
				.expect_log(log3(
					Precompile,
		...
		.execute_with(|| {
			precompiles()
				.prepare_test(
					Account::Alice,
					Account::Precompile,
					PCall::transfer { to: Address(Account::Bob.into()), value: 400.into() },
				)
		...
		.execute_with(|| {
			precompiles()
				.prepare_test(
					Account::Alice,
					Account::Precompile,
					PCall::transfer { to: Address(Account::Bob.into()), value: 1400.into() },
				)
		...
		.execute_with(|| {
			precompiles()
				.prepare_test(
					Account::Alice,
					Account::Precompile,
					PCall::approve { spender: Address(Account::Bob.into()), value: 300.into() },
				)
				.execute_some();
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			precompiles()
				...
				.expect_no_logs()
				.execute_returns(
					EvmDataWriter::new().write::<UnboundedBytes>("Mock token".into()).build(),
				);
		});
}
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			precompiles()
				...
				.expect_no_logs()
				.execute_returns(
					EvmDataWriter::new().write::<UnboundedBytes>("MOCK".into()).build(),
				);
		});
}
		...
		.execute_with(|| {
			...
			precompiles()
				.prepare_test(
					Account::Alice,
					Account::Precompile,
					PCall::balance_of { owner: Address(Account::Precompile.into()) },
				)
				.expect_cost(0) // TODO: Test db read/write costs
				.expect_no_logs()
			...
			assert_eq!(
				events(),
				vec![
					Event::System(frame_system::Event::NewAccount { account: Account::Precompile }),
					...
					Event::Evm(pallet_evm::Event::Executed { address: Account::Precompile.into() }),
				]
			);
		...
		.execute_with(|| {
			...
			precompiles()
				.prepare_test(
					Account::Alice,
					Account::Precompile,
					PCall::eip2612_nonces { owner: Address(Account::Alice.into()) },
				)
				.expect_cost(0) // TODO: Test db read/write costs
				.expect_no_logs()
